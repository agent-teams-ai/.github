...
const workflowPath = ".github/workflows/docs-qualification-authority-evolution-v5.yml";
const testPath = "scripts/docs-qualification-authority-evolution-v5.test.mjs";
...
const canonical = [
  ["docs/decisions/0003-reconciled-package-publication.md", "added", "63ddbca8b10b866ed9ed681d15ea9d2c8d5fcb2b"],
  ["docs/decisions/README.md", "modified", "c4d8237514994b4ea15f8273ad574740ec87ac5e"],
  ["governance/docs-qualified-cohorts.schema.json", "modified", "b3f8863091b6c4061566473c8e985845b1746dcd"],
  ["scripts/docs-cohort-policy.test.mjs", "modified", "cd7180d045288920f91e1b468d00d992a7b08a5f"],
  ["scripts/verify-docs-cohort-evidence.mjs", "modified", "f62064c99395d8f83186af4a093ecb4e30904895"],
...
test("accepts only the exact release-reconciliation authority slice", async () => {
  ...
  for (const candidate of [
    ...
    { files: canonical.map((file, index) => index === 0 ? { ...file, status: "modified" } : file) },
    { files: canonical.map((file, index) => index === 0 ? { ...file, previous_filename: "old.md" } : file) },
...
test("rejects every partial, missing, or wrong-blob authority slice", async () => {
  for (const [index, file] of canonical.entries()) {
    if (file.filename.startsWith("scripts/") || file.filename.endsWith(".schema.json")) {
