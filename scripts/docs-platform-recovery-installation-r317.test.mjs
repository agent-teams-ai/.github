...
function fixture(stage = "E", direction = "forward") {
  ...
  const protections = [{ id: 19979783, enforcement: "active", required: "synthetic exact tuple" }];
  ...
  const retainedForward = direction === "inverse" ? { ...structuredClone(accepted),
    direction: "forward", base: "c".repeat(40), head: base, decision_comment_id: 9317,
  ...
  const api = {
    getDecisionComment: async (id) => ({ id, user: { id: 42,
      ...
      issue_url: `https://api.github.com/repos/${repo.full_name}/issues/${accepted.pull_number}` }),
    ...
    getTree: async (revision) => gitTree(revision === base ? before :
      revision === "c".repeat(40) ? retainedBase : after, revision),
  ...
  return { event, accepted, api, pull, before, after, bodies, protections, rebind,
...
for (const [label, change, pattern] of [
  ...
  ["changed protection", (f) => { f.protections.push({ extra: true }); }, /protection snapshot/u],
