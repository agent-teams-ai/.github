...
const workflowPath = ".github/workflows/docs-qualification-authority-evolution-v11.yml";
const testPath = "scripts/docs-qualification-authority-evolution-v11.test.mjs";
const reviewedPath = "scripts/docs-cohort-policy.test.mjs";
const reviewedBlobSha = "217b6b6cfcfe0f208f3f7f30fbe948d5c3c5b0e4";
...
const exactFile = Object.freeze({
  filename: reviewedPath,
  status: "modified",
  sha: reviewedBlobSha,
});

async function classify(options = {}) {
  const files = options.files ?? [exactFile];
...
test("accepts the exact reviewed PR176 test blob and literal identity", async () => {
  const acceptedBlobLiteral = /const expectedBlobSha = "(?<sha>[0-9a-f]{40})";/u
    .exec(source)?.groups?.sha;
  assert.equal(acceptedBlobLiteral, reviewedBlobSha);
...
test("rejects wrong blob, status, rename metadata, extra files, and partial evidence", async () => {
  for (const files of [
    [{ ...exactFile, sha: "f".repeat(40) }],
    [{ ...exactFile, status: "added" }],
    [{ ...exactFile, previous_filename: "scripts/retired-policy.test.mjs" }],
    [exactFile, { filename: "docs/extra.md", status: "added", sha: "e".repeat(40) }],
  ]) {
  ...
  const incomplete = await classify({ paginated: [], changedFiles: 1 });
...
test("treats documentation, registry data, and runtime closures as noop", async () => {
...
test("keeps the successor base-owned, pinned, read-only, and non-executing", () => {
  assert.match(workflow, /^name: Trusted Docs Qualification Authority Evolution V11$/mu);
  ...
  assert.match(
    workflow,
    /trusted-qualification-authority-evolution-v11:\n    name: trusted-qualification-authority-evolution-v11/u,
  );
  assert.deepEqual(
