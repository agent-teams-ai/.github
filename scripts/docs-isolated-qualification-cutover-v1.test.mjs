...
const workflowPath = ".github/workflows/docs-isolated-qualification-cutover-v1.yml";
const testPath = "scripts/docs-isolated-qualification-cutover-v1.test.mjs";
...
const tuple = [
  ["scripts/verify-docs-consumer-gate.mjs", "002f5b5841768426b11529679a2962a6860e94fc", "c416a963f83078cb5016c9693a65ce5f3a7bfe4d"],
  ["scripts/verify-docs-consumer-gate.test.mjs", "d0bea695cdacd7556267c491bd09afb97adc84b1", "35bccd46a95f28697fc6b3183c148008aaee7c47"]
];
...
function fixture(base = "a".repeat(40), head = "b".repeat(40)) {
  const pull = { id: 1234, number: 203, state: "open", draft: false, merged: false,
    changed_files: 2, commits: 2, updated_at: "2026-09-05T12:00:00Z",
    base: { ref: "main", sha: base, repo: clone(identity) },
    head: { ref: "fix/isolated-qualification-lockfile", sha: head, repo: clone(identity) } };
  ...
  return { base, head, context: { eventName: "pull_request_target", sha: base, ref: "refs/heads/main",
    repo: { owner: "agent-teams-ai", repo: ".github" }, payload: { action: "synchronize", number: 203,
...
async function run(f = fixture(), alter = () => {}) {
  ...
  const github = { rest: {
    pulls: {
      get: api("pulls.get", (args, n) => { assert.equal(args.pull_number, 203); return f.pulls[n]; }),
      listFiles: api("pulls.listFiles", (args) => {
        assert.equal(args.pull_number, 203); assert.equal(args.per_page, 100); return f.pages[args.page - 1];
      })
    },
    repos: {
...
test("accepts the complete tuple at refreshed immutable base/head and emits exact evidence", async () => {
  for (const [base, head] of [["a".repeat(40), "b".repeat(40)], ["c".repeat(40), "d".repeat(40)]]) {
    ...
    assert.deepEqual(JSON.parse(f.outputs[0][1]), { repository: identity.full_name, repositoryId: identity.id,
      pr: 203, base, head, files: tuple.map(([path, oldBlob, newBlob]) => ({ path, oldBlob, newBlob, status: "modified", mode: "100644" })) });
...
test("accepts backward-only links on an empty out-of-range confirmation page", async () => {
  await run(fixture(), (method, response, occurrence) => {
    if (method === "pulls.listFiles" && occurrence === 1) response.headers.link =
      '<https://api.github.com/repos/agent-teams-ai/.github/pulls/203/files?per_page=100&page=1>; rel="prev", ' +
      '<https://api.github.com/repos/agent-teams-ai/.github/pulls/203/files?per_page=100&page=1>; rel="first"';
  });
});
...
for (const count of [0, -1, 1, 3, 1.5, "2", null, Number.MAX_SAFE_INTEGER + 1]) {
...
const mutations = {
  ...
  "wrong execution ref": (f) => { f.context.ref = "refs/pull/203/merge"; },
...
test("workflow has one unconditional base-owned pinned read-only step and no secrets/shell", () => {
  assert.equal(header, `name: Trusted Docs Isolated Qualification Cutover V1\n\non:\n  pull_request_target:\n    types: [opened, synchronize, reopened, edited, ready_for_review]\n\npermissions:\n  contents: read\n  pull-requests: read\n\njobs:\n  trusted-isolated-qualification-cutover-v1:\n    name: trusted-isolated-qualification-cutover-v1\n    runs-on: ubuntu-24.04\n    timeout-minutes: 3\n    steps:\n      - uses: actions/github-script@ed597411d8f924073f98dfc5c65a23a2325f34cd # v8.0.0\n        with:\n`);
...
rejectCase("rejects synchronized non-PR203 snapshots", (f) => {
...
const pageOne = "https://api.github.com/repositories/1316243981/pulls/203/files?per_page=100&page=1";
...
const foreignTargets = {
  ...
  "wrong PR": pageOne.replace("pulls/203", "pulls/193"),
