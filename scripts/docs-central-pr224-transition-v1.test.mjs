...
const workflowPath = ".github/workflows/docs-central-pr224-transition-v1.yml";
const testPath = "scripts/docs-central-pr224-transition-v1.test.mjs";
...
const tuple = production.map((row) => {
  ...
  for (const side of ["old", "new"]) {
    const bytes = Buffer.from(`synthetic ${side} ${row.path}\n`);
...
const manifestDigest = (rows, number) => digest(JSON.stringify(["central-pr224-transition-manifest-v1",
...
test("production pins PR224 and all three exact old/new regular-file tuples", async () => {
  assert.equal(productionPR, 224);
  assert.equal(digest(tupleText), "fc30dc25e7de58d24a61670eb7362ab83710103d4290b1fe2e6e048571950f12");
...
test("complete synthetic three-file modified tuple emits exact evidence only after final reread", async () => {
  ...
  assert.deepEqual(evidence, { repository: identity.full_name, repositoryId: identity.id, pr, base: f.base, head: f.head,
    ...
    result: "exact_implementation_transition", files: tuple });
...
test("PR text is inert and workflow is a single unconditional pinned base-owned read-only step", async () => {
  ...
  await run(f);
  assert.equal(header, `name: Trusted Central PR224 Transition v1\n\non:\n  pull_request_target:\n    types: [opened, synchronize, reopened, edited, ready_for_review]\n\npermissions:\n  contents: read\n  pull-requests: read\n\njobs:\n  trusted-central-pr224-transition-v1:\n    name: trusted-central-pr224-transition-v1\n    runs-on: ubuntu-24.04\n    timeout-minutes: 3\n    steps:\n      - uses: actions/github-script@ed597411d8f924073f98dfc5c65a23a2325f34cd # v8.0.0\n        with:\n`);
...
rejectCase("rejects inverse new-to-old tuple even on bound PR", f => {
...
test("manifest covers both sides and lengths and is independent of tuple order", () => {
  const original = manifestDigest(production, 224);
  assert.equal(original, manifestDigest([...production].reverse(), 224));
  for (const side of ["old", "new"]) for (const field of ["mode", "type", "blob", "bytes", "sha256"]) {
    ...
    assert.notEqual(original, manifestDigest(changed, 224));
  }
  assert.notEqual(original, manifestDigest(production, 210));
});

test("production PR224 binding accepts the exact synthetic tuple through final reread", async () => {
  const f = fixture();
  f.context.payload.number = f.context.payload.pull_request.number = 224;
  f.pulls.forEach(pull => { pull.number = 224; });
  await run(f, undefined, compile(source.replace(`const tuple = ${tupleText};`, `const tuple = ${JSON.stringify(tuple)};`)));
  assert.equal(JSON.parse(f.outputs[0][1]).pr, 224);
  assert.equal(JSON.parse(f.outputs[0][1]).manifestSHA256, manifestDigest(tuple, 224));
  assert.deepEqual(f.calls.slice(-3).map(row => row.name), ["pulls.get", "repos.get", "repos.getBranch"]);
});
